...
path_zoo = r'E:\TongJiJianMo\百度指数数据\beijingdongwuyuan.xlsx'
path_ticket = r'E:\TongJiJianMo\百度指数数据\beijingdongwuyuanmenpiao.xlsx'
path_aqi = r'E:\TongJiJianMo\天气数据\北京空气质量数据.xls'
output_path = r'E:\TongJiJianMo\data\multi_data_for_MetaLSTM.xlsx'

# 确保输出目录存在
os.makedirs(r'E:\TongJiJianMo\data', exist_ok=True)

try:
    # 1.1 读取三个数据集
    df_zoo = pd.read_excel(path_zoo)[['日期', '全部']].rename(columns={'日期': 'date', '全部': 'zoo_index'})
    df_ticket = pd.read_excel(path_ticket)[['日期', '全部']].rename(columns={'日期': 'date', '全部': 'ticket_index'})
    df_aqi = pd.read_excel(path_aqi)[['时间', 'AQI']].rename(columns={'时间': 'date', 'AQI': 'AQI'})

    # 1.2 统一时间格式并设置为索引
    df_zoo['date'] = pd.to_datetime(df_zoo['date'])
    df_ticket['date'] = pd.to_datetime(df_ticket['date'])
    df_aqi['date'] = pd.to_datetime(df_aqi['date'])
    
    df_zoo.set_index('date', inplace=True)
    df_ticket.set_index('date', inplace=True)
    df_aqi.set_index('date', inplace=True)

    # 1.3 按照时间列进行外部合并 (Outer Join)，保证时间序列的连续性
    df_merged = df_zoo.join(df_ticket, how='outer').join(df_aqi, how='outer')
    
    # 截取有效数据段 (以AQI的最晚时间2026-04-17为准，保证数据完整性)
    df_merged = df_merged.loc['2019-04-01':'2026-04-17']

    # 1.4 【专业级缺失值处理】使用 time 模式插值，这在时间序列中比普通线性插值更严谨
    # 对于开头可能出现的缺失值，使用 bfill 兜底
    df_merged['zoo_index'] = pd.to_numeric(df_merged['zoo_index'], errors='coerce')
    df_merged['ticket_index'] = pd.to_numeric(df_merged['ticket_index'], errors='coerce')
    df_merged['AQI'] = pd.to_numeric(df_merged['AQI'], errors='coerce')
    ...
    # 1.6 【元认知模块】：利用主体目标计算历史误差纠错特征 (history_error)
    ...
    df_merged['history_error'].fillna(0, inplace=True) # 首部空值补0
    ...
    print(f"数据处理出错: {e}。请检查文件路径和表头是否完全匹配。")
    exit()
...
model.save('zoo_lstm_attention_traffic_model.h5')
print("已保存模型文件: zoo_lstm_attention_meta_traffic_model.h5")
